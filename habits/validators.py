...
class RelatedAndAwardValidator:
    ...
    def __call__(self, data):
        if data.get('related_habit') and data.get('award'):
            raise serializers.ValidationError("Нельзя одновременно указывать и связанную привычку, и вознаграждение. Выберите что-то одно.")
        return data
    ...
    def __call__(self, data):
        ...
        if time_execution and time_execution > 120:
            raise serializers.ValidationError({
                "time_execution": "Время выполнения не должно превышать 120 секунд."
            })
        return data
...
class PeriodicityValidator:
    ...
    def __call__(self, data):
        ...
        if periodicity:
            if periodicity < 1:
                raise serializers.ValidationError({
                    "periodicity": "Периодичность не может быть меньше 1 дня."
                })
            if periodicity > 7:
                raise serializers.ValidationError({
                    "periodicity": "Периодичность не может быть больше 7 дней."
                })
        return data
